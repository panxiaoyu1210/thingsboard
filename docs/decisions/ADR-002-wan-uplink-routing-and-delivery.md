...
协议中的 `req_id` 是普通整数，没有全局唯一性、生成方标识或时间窗口。NS 发起的上行可能复用 ThingsBoard 尚未完成请求的编号，也可能在设备持续上报时重复使用编号，因此不能把它当作可靠的上行幂等键。
...
每个有效 `push_uplink` 转换为一个 `PostTelemetryMsg`，包含字符串 `wanData` 和整数 `wanPort`、`rssi`、`snr`，并调用标准 `TransportService.process`。该入口会记录设备活动，按 Session 中的 Device Profile 路由规则链，再由现有规则链保存时序数据。第一版保留十六进制载荷，不解析业务字段。
