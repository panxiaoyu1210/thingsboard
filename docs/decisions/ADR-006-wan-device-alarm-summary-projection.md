...
系统部件通过 `activeAlarmCount` 的颜色范围控制单个标记：大于零时使用红色，零或缺失时保持网关蓝色或终端绿色。低缩放级别允许聚合，聚合标记只要任一子设备存在告警就使用红色，表示“该范围内存在告警”而不是全部设备均告警。聚合半径限制为 40px，缩放到 16 级及以上时自动拆分为单设备标记；单设备 Tooltip 使用悬浮触发并显示计数与最高等级。
...
- 地图在大范围视图保留聚合告警提示，在安装位置级视图自动拆分，避免聚合状态遮蔽具体告警设备。
